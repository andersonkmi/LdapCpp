...
LDAPConnectionVariableTimeoutPolicy::LDAPConnectionVariableTimeoutPolicy() : timeout_(120)
{

}

LDAPConnectionVariableTimeoutPolicy::~LDAPConnectionVariableTimeoutPolicy()
{

}

void LDAPConnectionVariableTimeoutPolicy::setBindTimeout(unsigned int timeout) {
	timeout_ = timeout;
}

void LDAPConnectionVariableTimeoutPolicy::setTimeout(LDAP* ldap) {
	int iRtn = ldap_set_option(ldap, LDAP_OPT_TIMELIMIT, (void*) &timeout_);

	if(iRtn != LDAP_SUCCESS) {
		std::ostringstream buffer;
		buffer << "LDAP timeout setting failed: ";
