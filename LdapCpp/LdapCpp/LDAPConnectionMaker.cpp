...
LDAPConnectionMaker::LDAPConnectionMaker() : timeout_(120) 
{

}

LDAPConnectionMaker::~LDAPConnectionMaker()
{

}

void LDAPConnectionMaker::setConnectionTimeout(unsigned long timeout) {
	timeout_ = timeout;
}

void LDAPConnectionMaker::connect(LDAP* ldap) {
	LDAP_TIMEVAL ldapTime;
	ldapTime.tv_sec = timeout_;
	ldapTime.tv_usec = 0;

	long returnCode = ldap_connect(ldap, &ldapTime);
	if(returnCode != LDAP_SUCCESS) {
		std::ostringstream buffer;
		buffer << "LDAP connection failed: ";
