...
LDAPConnection::LDAPConnection(LDAP* ldapConn) : ldap_(ldapConn), isBound_(false) {

}

LDAPConnection::~LDAPConnection(void) {
	if(isBound_) {
		ULONG returnCode = ldap_unbind_s(ldap_);
...
void LDAPConnection::bind(const std::string& uid, const std::string& credentials) {
	PCHAR dn = (PCHAR) uid.c_str();
	PCHAR pwd = (PCHAR) credentials.c_str();
	int iRtn = ldap_bind_s(ldap_, dn, pwd, LDAP_AUTH_SIMPLE);
	if (iRtn != LDAP_SUCCESS) {
		std::ostringstream buffer;
		buffer << "LDAP bind failed: ";
...
//LDAP* LDAPConnection::getConnectionHandle() {
//	return ldap_;
//}
}