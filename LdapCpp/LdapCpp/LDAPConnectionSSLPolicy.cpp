...
LDAPConnectionSSLPolicy::LDAPConnectionSSLPolicy()
{
...
LDAP* LDAPConnectionSSLPolicy::initialize(const std::string& server, const std::string& port) {
	PCHAR host = (PCHAR) server.c_str();
	ULONG portNum;
	if(port.compare("389") == 0) {
		portNum = LDAP_PORT;
	} else {
		long portNumber = atol(port.c_str());
		portNum = (ULONG) portNumber;
	}
	
	LDAP* ldap = ldap_sslinit(host, portNum, 1);
	if(ldap == NULL) {
		std::ostringstream buffer;
		buffer << "LDAP initialization failed for host = '" << server << "' and port = '" << port <<  "'" << ":";
	...
	int rc = 0;
	rc = ldap_set_option(ldap, LDAP_OPT_SERVER_CERTIFICATE, &serverCertRoutine); 
	if(rc != LDAP_SUCCESS) {
		std::ostringstream message;
		message << "Error when configuring server certificate validator";
		throw LDAPOtherException(message.str());
	}
...
BOOLEAN _cdecl serverCertRoutine (PLDAP connection, PCCERT_CONTEXT pServerCert) { 
	if(pServerCert != NULL) {
