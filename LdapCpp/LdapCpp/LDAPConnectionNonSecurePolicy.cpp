...
LDAPConnectionNonSecurePolicy::LDAPConnectionNonSecurePolicy()
{
...
LDAP* LDAPConnectionNonSecurePolicy::initialize(const std::string& server, const std::string& port) {
	...
	LDAP* ldap = ldap_init(host, portNum);
	if(ldap == NULL) {
