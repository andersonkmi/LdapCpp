...
const std::string ActiveDir2003ServiceProvider::SAMACCOUNT_NAME_ATTR = "sAMAccountName";
const std::string ActiveDir2003ServiceProvider::USER_ACCOUNT_CONTROL_ATTR = "userAccountControl";

ActiveDir2003ServiceProvider::ActiveDir2003ServiceProvider() {
...
boost::shared_ptr<LDAPUser> ActiveDir2003ServiceProvider::searchUser(const std::string& user, const std::string& baseDN) throw (LDAPException) {
	std::ostringstream buffer;
	buffer << "(&(" << SAMACCOUNT_NAME_ATTR << "=" << user << ")(" << LDAPAuthServiceProvider::OBJECT_CLASS_ATTR << "=" << LDAPAuthServiceProvider::OBJECT_CLASS_ATTR_PERSON_VALUE << "))";
	std::string searchFilter = buffer.str();
	PCHAR filter = const_cast<char*>(searchFilter.c_str());

	PCHAR resultAttributes[3];
	resultAttributes[0] = const_cast<char*>(LDAPAuthServiceProvider::DISPLAY_NAME_ATTR.c_str());
	resultAttributes[1] = const_cast<char*>(USER_ACCOUNT_CONTROL_ATTR.c_str());
	resultAttributes[2] = NULL;
	...
	std::string userAccountControl = "";

	std::string displayNameAttr = LDAPAuthServiceProvider::DISPLAY_NAME_ATTR;
	std::transform(displayNameAttr.begin(), displayNameAttr.end(), displayNameAttr.begin(), ::tolower);

	std::string userAccountControlAttr = USER_ACCOUNT_CONTROL_ATTR;
	std::transform(userAccountControlAttr.begin(), userAccountControlAttr.end(), userAccountControlAttr.begin(), ::tolower);

	while(pAttribute != NULL) {
		std::ostringstream attrNameBuffer;
		attrNameBuffer << pAttribute;
		...
		if(ppValue != NULL) {
			...
			if(iValue != NULL)
			{
				std::ostringstream attrValueBuffer;
				attrValueBuffer << *ppValue;
				std::string attributeName(attrNameBuffer.str());
				std::transform(attributeName.begin(), attributeName.end(), attributeName.begin(), ::tolower);
				if(attributeName.compare(displayNameAttr) == 0) {
					displayName = attrValueBuffer.str();
				} else if(attributeName.compare(userAccountControlAttr) == 0) {
					userAccountControl = attrValueBuffer.str();
				}
			}
		} 
	...
	unsigned int userAccountControlNumber = 0;
	std::istringstream input(userAccountControl);
	if(!(input >> userAccountControlNumber)) {
		userAccountControlNumber = LDAPUser::ACCOUNTDISABLE;
	}

	boost::shared_ptr<LDAPUser> ldapUser(new LDAPUser(user, userDN, displayName, userAccountControlNumber));
	return ldapUser;
}

void ActiveDir2003ServiceProvider::validateUser(const boost::shared_ptr<LDAPUser>& user) throw(LDAPUserAccountDisableException, LDAPUserAccountLockoutException, LDAPUserAccountPasswordExpiredException, LDAPInvalidUserAccountException) {
	unsigned int userAccountControl = user->getUserAccountControl();

	if((userAccountControl & LDAPUser::ACCOUNTDISABLE) == LDAPUser::ACCOUNTDISABLE) {
		std::ostringstream message;
		message << "User account '" << user->getUid() << "' is disabled. Code returned: " << userAccountControl;
		throw LDAPUserAccountDisableException(message.str());
	} else if((userAccountControl & LDAPUser::LOCKOUT) == LDAPUser::LOCKOUT) {
		std::ostringstream message;
		message << "User account '" << user->getUid() << "' is locked out. Code returned: " << userAccountControl;
		throw LDAPUserAccountLockoutException(message.str());
	} else if((userAccountControl & LDAPUser::PASSWORD_EXPIRED) == LDAPUser::PASSWORD_EXPIRED) {
		std::ostringstream message;
		message << "User account '" << user->getUid() << "' password is expired. Code returned: " << userAccountControl;
		throw LDAPUserAccountPasswordExpiredException(message.str());
	} else if((userAccountControl & LDAPUser::NORMAL_ACCOUNT) != LDAPUser::NORMAL_ACCOUNT) {
		std::ostringstream message;
		message << "User account '" << user->getUid() << "' is not a normal account. Code returned: " << userAccountControl;
